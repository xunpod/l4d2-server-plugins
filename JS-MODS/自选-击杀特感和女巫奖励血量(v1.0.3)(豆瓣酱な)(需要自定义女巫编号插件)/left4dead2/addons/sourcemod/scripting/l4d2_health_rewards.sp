...
public void Event_Witchkilled(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(g_bWitchNumber == true && IsValidClient(client) && GetClientTeam(client) == 2)
	{
		char sType[32];
		int iReward;
		int iWitchid = event.GetInt("witchid");
	...
	if(IsValidClient(attacker) && GetClientTeam(attacker) == 2)
	{
		if(IsValidClient(client) && GetClientTeam(client) == 3)
		{
			...
			if(IsPlayerAlive(attacker) || IsPlayerState(attacker))
			{
				...
				{
					if(iReward > 0)
						PrintToChat(iBot != 0 ? iBot : attacker, "\x04[提示]\x05%s了\x03%s\x04,\x05奖励\x03%d\x05点血量.", sType, GetPlayerName(client, iHLZClass), GetRewardHealth(iReward));
				}
				else
				{
					if(iReward > 0)
						PrintToChat(iBot != 0 ? iBot : attacker, "\x04[提示]\x05%s了\x03%s\x04,\x05血量已达\x03%d\x05上限.", sType, GetPlayerName(client, iHLZClass), g_iLimitHealth);//聊天窗提示.
				}
			}
			else
			{
				if(iReward > 0)
					PrintToChat(iBot != 0 ? iBot : attacker, "\x04[提示]\x05%s了\x03%s\x04.", sType, GetPlayerName(client, iHLZClass));//聊天窗提示.
			}
		}
	}
}
