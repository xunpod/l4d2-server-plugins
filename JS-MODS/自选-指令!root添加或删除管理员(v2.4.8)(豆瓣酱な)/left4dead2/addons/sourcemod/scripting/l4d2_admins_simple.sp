...
#define PLUGIN_VERSION	"2.4.8"	//版本号.
...
{
	char sPassword[64], sRandom[8][32];
	for (int i = 0; i < sizeof(sRandom); i++)
		IntToString(GetRandomInt(0, 9),	sRandom[i], sizeof(sRandom[]));
	ImplodeStrings(sRandom, sizeof(sRandom), "", sPassword, sizeof(sPassword));//打包字符串.
	return sPassword;
}
