...
public OnPluginStart()
{
	...
	// version cvar
	CreateConVar( "sm_skill_detect_version", PLUGIN_VERSION, "插件版本", FCVAR_NOTIFY|FCVAR_REPLICATED|FCVAR_DONTRECORD );
	...
	g_hCvarAllowGLSkeet = CreateConVar(		"sm_skill_skeet_allowgl",		"1", "是否检测榴弹击杀飞扑 Hunter", FCVAR_NONE, true, 0.0, true, 1.0 );
	g_hCvarDrawCrownThresh = CreateConVar(	"sm_skill_drawcrown_damage",  "500", "需要造成多少伤害才视为引秒 Witch", FCVAR_NONE, true, 0.0, false );
	g_hCvarSelfClearThresh = CreateConVar(	"sm_skill_selfclear_damage",  "200", "需要造成多少伤害才视为被拉自救 (Smoker 舌头自救)", FCVAR_NONE, true, 0.0, false );
	g_hCvarHunterDPThresh = CreateConVar(	"sm_skill_hunterdp_height",	  "400", "需要多高才算是 Hunter 高扑", FCVAR_NONE, true, 0.0, false );
	g_hCvarJockeyDPThresh = CreateConVar(	"sm_skill_jockeydp_height",	  "300", "需要多高才算是 Jockey 空降", FCVAR_NONE, true, 0.0, false );
	g_hCvarHideFakeDamage = CreateConVar(	"sm_skill_hidefakedamage",		"0", "是否隐藏溢出伤害", FCVAR_NONE, true, 0.0, true, 1.0 );
	g_hCvarDeathChargeHeight = CreateConVar("sm_skill_deathcharge_height","400", "需要多高才算是 Charger 冲锋秒杀", FCVAR_NONE, true, 0.0, false );
	g_hCvarInstaTime = CreateConVar(		"sm_skill_instaclear_time",		"0.75", "在多少秒内救出被控队友才显示", FCVAR_NONE, true, 0.0, false );
	g_hCvarBHopMinStreak = CreateConVar(	"sm_skill_bhopstreak",			"3", "连跳需要跳多少下才进行检测", FCVAR_NONE, true, 0.0, false );
	g_hCvarBHopMinInitSpeed = CreateConVar( "sm_skill_bhopinitspeed",	  "150", "连跳第一下需要达到多少速度才视为有效连跳", FCVAR_NONE, true, 0.0, false );
	g_hCvarBHopContSpeed = CreateConVar(	"sm_skill_bhopkeepspeed",	  "300", "连跳最小速度，小于这个不会视为连跳", FCVAR_NONE, true, 0.0, false );
...
public Action: Event_PlayerShoved( Handle:event, const String:name[], bool:dontBroadcast )
{
	...
	if ( zClass == ZC_BOOMER )
	{
		...
		{
			HandlePopStop(attacker, victim, g_iBoomerVomitHits[victim], FloatSub(GetGameTime(), g_fBoomerVomitStart[victim]));
	...
	if ( !biled && !g_bBoomerHitSomebody[client] && g_bBoomerNearSomebody[client] )
	{
		...
		if ( IS_VALID_SURVIVOR(attacker) )
		{
			HandlePop( attacker, client, g_iBoomerGotShoved[client],
				FloatSub(GetGameTime(), g_fSpawnTime[client]),
				FloatSub(GetGameTime(), g_fBoomerNearTime[client]) );
		}
	}
}
...
stock HandlePop( attacker, victim, shoveCount, Float:timeAlive, Float:timeNear )
{
	...
	if ( GetConVarBool(g_hCvarReport) && (GetConVarInt(g_hCvarReportFlags) & REP_POP) && timeNear < 5.0 )
	{
		...
		{
			PrintToChatAll( "\x03☆ \x01玩家 \x04%N\x01 把接近生还者的 \x05%N\x01 安全地解决了. (%.1f 秒)", attacker, victim, timeNear );
		}
		else if ( IS_VALID_INGAME(attacker) )
		{
			PrintToChatAll( "\x03☆ \x01玩家 \x04%N\x01 把接近生还者的 \x05Boomer\x01 安全地解决了. (%.1f 秒)", attacker, timeNear );
		}
	}
...
stock HandlePopStop(attacker, victim, hits, Float:timeVomit)
{
		...
		hits < 1 && timeVomit < GetConVarFloat(g_hCvarInstaTime) )
	{
		if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
		{
			PrintToChatAll( "\x03☆ \x01玩家 \x04%N\x01 把正在呕吐的 \x05%N\x01 推停了. (%.1f 秒)", attacker, victim, timeVomit );
		}
		else if ( IS_VALID_INGAME(attacker) )
		{
...
stock HandleLevel( attacker, victim )
{
	...
	if ( GetConVarBool(g_hCvarReport) && (GetConVarInt(g_hCvarReportFlags) & REP_LEVEL) )
	{
		...
		{
			PrintToChatAll( "\x03★ \x01某个 \x04Charger\x01 被 \x04生还者\x01 近战秒杀了" );
		}
	}
...
stock HandleLevelHurt( attacker, victim, damage )
{
	...
	if ( GetConVarBool(g_hCvarReport) && (GetConVarInt(g_hCvarReportFlags) & REP_HURTLEVEL) )
	{
		if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
		{
			PrintToChatAll( "\x03☆ \x04%N\x01 使用近战砍死了冲锋的 \x05%N\x01 (伤害 \x03%i\x01)", attacker, victim, damage );
		}
		else if ( IS_VALID_INGAME(attacker) )
		{
			PrintToChatAll( "\x03☆ \x04%N\x01 使用近战砍死了冲锋的 \x05Charger\x01. (伤害 \x03%i\x01)", attacker, damage );
		}
		else {
...
stock HandleDeadstop( attacker, victim, bool:hunter = true )
{
	...
	if ( GetConVarBool(g_hCvarReport) && (GetConVarInt(g_hCvarReportFlags) & REP_DEADSTOP) )
	{
		if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
		{
			PrintToChatAll( "\x03☆ \x04%N\x01 推停了飞扑的 \x05%N\x01", attacker, victim );
		}
		else if ( IS_VALID_INGAME(attacker) )
		{
			if(hunter)
				PrintToChatAll( "\x03☆ \x04%N\x01 推停了飞扑的 \x05Hunter\x01", attacker );
			else
				PrintToChatAll( "\x03☆ \x04%N\x01 推停了飞扑的 \x05Jockey\x01", attacker );
		}
	}
...
stock HandleShove( attacker, victim, zombieClass )
{
	...
	if ( GetConVarBool(g_hCvarReport) && (GetConVarInt(g_hCvarReportFlags) & REP_SHOVE) )
	{
		if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
		{
			PrintToChatAll( "\x03☆ \x04%N\x01 推开了尝试控人的 \x05%N\x01", attacker, victim );
		}
		else if ( IS_VALID_INGAME(attacker) )
		{
...
stock HandleSkeetAssist(attacker, victim)
{
	...
	if(victim > 0 && victim <= MaxClients && g_iHunterShotDmgTeam[victim] > 0)
	{
		...
		for(new i = 1; i <= MaxClients; ++i)
		{
			...
			if(assistMsg[0] == EOS)
				strcopy(assistMsg, 255, "\x03助攻：\x01");
			else
				StrCat(assistMsg, 255, "，");
	...
	shots = 1, assist = -1, bool:isHunter = true )
{
	...
	if ( GetConVarBool(g_hCvarReport) && (GetConVarInt(g_hCvarReportFlags) & REP_SKEET) )
	{
		if ( attacker == -2 )
		{
			...
			{
				if ( IS_VALID_INGAME(victim) && !IsFakeClient(victim) ) {
					PrintToChatAll( "\x03★ \x05%N\x01 在飞扑时被多人合力打死了", victim );
				} else {
					PrintToChatAll( "\x03★ \x01 某个 \x05%s\x01 在飞扑时被多人合力打死了",
		...
		{
			if(bMelee)
				PrintToChatAll("\x03★ \x04%N\x01 近战秒了飞扑的 \x05%N\x01", attacker, victim);
			else if(bSniper)
				PrintToChatAll("\x03★ \x04%N\x01 狙击空爆了飞扑的 \x05%N\x01", attacker, victim);
			else if(bGL)
			...
			/*
			PrintToChatAll( "\x03[提示] \x04%N\x01 %s 杀死了飞扑的 \x05%N\x01",
					attacker,
					(bMelee) ? "近战": ((bSniper) ? "爆头" : ((bGL) ? "榴弹" : "") ),
					victim 
				);
			*/
		}
		...
		{
			/*
			PrintToChatAll( "\x03[提示] \x04%N\x01 %s 杀死了飞扑的 \x05Hunter\x01",
					attacker,
					(bMelee) ? "近战": ((bSniper) ? "爆头" : ((bGL) ? "榴弹" : "") )
				);
			*/
			...
			if(bMelee)
				PrintToChatAll("\x03★ \x04%N\x01 近战秒了飞扑的 \x05%s\x01", attacker, (isHunter ? "Hunter" : "Jockey"));
			else if(bSniper)
				PrintToChatAll("\x03★ \x04%N\x01 狙击空爆了飞扑的 \x05%s\x01", attacker, (isHunter ? "Hunter" : "Jockey"));
			else if(bGL)
	...
	shots = 1, bool:isHunter = true )
{
	...
	if ( GetConVarBool(g_hCvarReport) && (GetConVarInt(g_hCvarReportFlags) & REP_HURTSKEET) )
	{
		if(IS_VALID_INGAME(attacker))
		{
			...
			{
				// PrintToChatAll( "\x03[提示] \x05%N\x01 在飞扑时被打死了 (伤害 \x03%i\x01).%s", victim, damage, (bOverKill) ? "(可能会触发空中击杀)" : "" );
				
				if(bMelee)
				{
					PrintToChatAll("\x03%s \x05%N\x01 砍死了正在飞扑的 \x04%N\x01 (伤害 \x03%d\x01)",
						(bOverKill ? "★" : "☆"), attacker, victim, damage);
				}
				else if(shots > 1)
				{
					PrintToChatAll("\x03%s \x05%N\x01 射死了正在飞扑的 \x04%N\x01 (射击 \x03%d\x01 次, 伤害 \x03%d\x01)",
						(bOverKill ? "★" : "☆"), attacker, victim, shots, damage);
				}
				else
				{
			...
			{
				// PrintToChatAll( "\x03[提示] \x01Hunter 在飞扑时被打死了 (伤害 \x03%i\x01).%s", damage, (bOverKill) ? "(可能会触发空中击杀)" : "" );
				
				if(bMelee)
				{
					PrintToChatAll("\x03%s \x05%N\x01 砍死了正在飞扑的 \x04%s\x01 (伤害 \x03%d\x01)",
						(bOverKill ? "★" : "☆"), attacker, (isHunter ? "Hunter" : "Jockey"), damage);
				}
				else if(shots > 0)
				{
				...
				{
					PrintToChatAll("\x03%s \x05%N\x01 打死了正在飞扑的 \x04%s\x01 (伤害 \x03%d\x01)",
						(bOverKill ? "★" : "☆"), attacker, (isHunter ? "Hunter" : "Jockey"), damage);
				}
			}
...
HandleCrown( attacker, damage )
{
	...
	if ( GetConVarBool(g_hCvarReport) && (GetConVarInt(g_hCvarReportFlags) & REP_CROWN) )
	{
		if ( IS_VALID_INGAME(attacker) )
		{
			PrintToChatAll( "\x03☆ \x04%N\x01 秒杀了一个 \x05Witch\x01 (伤害 \x03%i\x01)", attacker, damage );
		}
		else {
            A=1;
			//PrintToChatAll( "\x03☆ \x01某个 \x05Witch\x01 被秒掉了" );
		}
	}
...
HandleDrawCrown( attacker, damage, chipdamage )
{
	...
	if ( GetConVarBool(g_hCvarReport) && (GetConVarInt(g_hCvarReportFlags) & REP_DRAWCROWN) )
	{
		if ( IS_VALID_INGAME(attacker) )
		{
			...
			PrintToChatAll( "\x03★ \x01未知人士 引秒了一个 \x05Witch\x01 (最终伤害 \x03%i\x01 damage, 初始伤害 \x05%i\x01)", damage, chipdamage );
		}
	}
...
HandleTongueCut( attacker, victim )
{
	...
	if ( GetConVarBool(g_hCvarReport) && (GetConVarInt(g_hCvarReportFlags) & REP_TONGUECUT) )
	{
		...
		{
			PrintToChatAll( "\x03☆ \x04%N\x01 近战砍断了 \x05Smoker\x01 的舌头", attacker );
		}
	}
...
HandleSmokerSelfClear( attacker, victim, bool:withShove = false )
{
	...
	if (	GetConVarBool(g_hCvarReport) && (GetConVarInt(g_hCvarReportFlags) & REP_SELFCLEAR) &&
		...
		if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
		{
			PrintToChatAll( "\x03☆ \x04%N\x01 被 \x05%N\x01 拉后自救%s", attacker, victim, (withShove ? " (推)" : "") );
		}
		else if ( IS_VALID_INGAME(attacker) )
		{
			PrintToChatAll( "\x03☆ \x04%N\x01 被 \x05Smoker\x01 拉后自救%s", attacker, (withShove ? " (推)" : "") );
		}
	}
...
stock HandleHunterDP( attacker, victim, actualDamage, Float:calculatedDamage, Float:height, bool:playerIncapped = false )
{
	...
	if (	GetConVarBool(g_hCvarReport)
		...
		{
			PrintToChatAll( "\x03★ \x04%N\x01 高空飞扑砸到了 \x05%N\x01 (伤害 \x03%i\x01, 高度 \x05%i\x01)", attacker,  victim, RoundFloat(calculatedDamage), RoundFloat(height) );
		}
...
stock HandleJockeyDP( attacker, victim, Float:height )
{
	...
	if (	GetConVarBool(g_hCvarReport)
		...
		if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(attacker) )
		{
			PrintToChatAll( "\x03★ \x04%N\x01 空投骑到了 \x05%N\x01 的脸上 (高度 \x05%i\x01)", attacker,  victim, RoundFloat(height) );
		}
		else if ( IS_VALID_INGAME(victim) )
		{
	...
	if ( attacker != pinVictim && GetConVarBool(g_hCvarReport) && (GetConVarInt(g_hCvarReportFlags) & REP_INSTACLEAR) )
	{
		...
		if ( fClearTime != -1.0 && fClearTime <= fMinTime )
		{
			if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
			{
				...
				{
					PrintToChatAll( "\x03☆ \x04%N\x01 在 \x05%.2f\x01 秒内从 \x04%N\x01 (%s) 手里救出了被控的 \x05%N\x01",
						attacker, fClearTime, victim, g_csSIClassName[zombieClass], pinVictim);
				} else {
					PrintToChatAll( "\x03☆ \x04%N\x01 在 \x05%.2f\x01 秒内从 \x05%N\x01 (%s) 手里救出了被控的\x04队友\x01",
						attacker, fClearTime, victim, g_csSIClassName[zombieClass]);
				}
			}
			else if ( IS_VALID_INGAME(attacker) )
			{
				if ( IS_VALID_INGAME(pinVictim) )
				{
					PrintToChatAll( "\x03☆ \x04%N\x01 在 \x05%.2f\x01 秒内从特感 \x05%s\x01 手里救出了被控的队友 \x05%N\x01",
						attacker, fClearTime, g_csSIClassName[zombieClass], pinVictim);
				} else {
					PrintToChatAll( "\x03☆ \x04%N\x01 在 \x05%.2f\x01 秒内从特感 \x05%s\x01 手里救出了被控的\x04队友\x01",
						attacker, fClearTime, g_csSIClassName[zombieClass]);
				}
			}
		}
	}
...
stock HandleBHopStreak( survivor, streak, Float: maxVelocity )
{
	if (	GetConVarBool(g_hCvarReport) && (GetConVarInt(g_hCvarReportFlags) & REP_BHOPSTREAK) &&
		...
		PrintToChatAll( "\x03☆ \x04%N\x01 连跳了 \x05%i\x01 次 (最高速度 \x05%.1f\x01)",
				survivor,
				streak,
				maxVelocity
			);
...
stock HandleCarAlarmTriggered( survivor, infected, reason )
{
	if (	GetConVarBool(g_hCvarReport) && (GetConVarInt(g_hCvarReportFlags) & REP_CARALARM) &&
		...
		if ( reason == CALARM_HIT ) {
			PrintToChatAll( "\x03× \x05%N\x01 攻击了警报车", survivor );
		}
		else if ( reason == CALARM_TOUCHED )
		{
			...
			if ( IS_VALID_INFECTED(infected) )
			{
				...
				{
					PrintToChatAll( "\x03× \x04%N\x01 携带 \x05%N\x01 触碰了警报车", infected, survivor );
				}
				else {
					switch ( GetEntProp(infected, Prop_Send, "m_zombieClass") )
					{
						case ZC_SMOKER: { PrintToChatAll( "\x03× \x05%N\x01 因为被 \x04Hunter\x01 突袭而触碰了警报车", survivor ); }
						case ZC_JOCKEY: { PrintToChatAll( "\x03× \x05%N\x01 因为被 \x04Jockey\x01 骑而触碰了警报车", survivor ); }
						case ZC_CHARGER: { PrintToChatAll( "\x03× \x05%N\x01 因为被 \x04Charger\x01 撞而触碰了警报车", survivor ); }
						default: { PrintToChatAll( "\x03× \x05%N\x01 因为被某个特感撸了而触碰了警报车", survivor ); }
					}
				}
			}
			else
			{
				PrintToChatAll( "\x03× \x05%N\x01 触碰了警报车", survivor );
			}
		}
		else if ( reason == CALARM_EXPLOSION ) {
			PrintToChatAll( "\x03× \x05%N\x01 制造爆炸触发了警报车", survivor );
		}
		else if ( reason == CALARM_BOOMER )
		{
			if ( IS_VALID_INFECTED(infected) && !IsFakeClient(infected) )
			{
				PrintToChatAll( "\x03× \x05%N\x01 因为打死 \x04%N\x01 (Boomer) 而触发了警报车", survivor, infected );
			}
			else
			{
				PrintToChatAll( "\x03× \x05%N\x01 因为打死 \x04Boomer\x01 而触发了警报车", survivor );
			}
		}
		else {
